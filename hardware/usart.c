...
unsigned char transmit(const char *str, ...){
	...
	while(token != \0){
		if( *token != '%')
			...
			switch(*token++){
				case 'c': _transmit(va_args(tokens, int));
					break;
				case 'd': _transmit(va_args(tokens, int)+'0');
					break;
				case 's':transmit(va_args(tokens, char*));	// carefully observe that this line calls the function again.
					break;
			}
		}
		token++;
...
unsigned char *receive(void){
	char *input_string;
	char token;
	input_string = (unsigned char) malloc(sizeof(MAXLEN)); // If this line is not given, we will get a segfault
	int i=0;
	token = _receive();
	while(token != NULL){
